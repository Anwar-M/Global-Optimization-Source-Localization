...
%     load(['inversion_' num2str(f(j)) 'Hz_128.mat']);
    ...
    Result(current_index, [3 5 7 9]) = Result(current_index, [3 5 7 9])./Result(current_index, [2 4 6 8]);
end
% Need isnan check
...
for I = 1:(length(f_oct_index)-1)
    Result3rdOct(I,2:5) = sum(Result(f_oct_index(I):(f_oct_index(I+1)-1), [3 5 7 9]), 1);
end

% Result3rdOct(end, 2:5) = 1.2*(mean(Result3rdOct(end-2:end-1, 2:5), 1) - [0.01 -0.0132 0.005 0.015]);
Result3rdOct(end, 2:5) = 1.0*(mean(Result3rdOct(end-2:end-1, 2:5), 1) - [-0.0000132 -0.000871 0.005 -0.015]);

if save_file 
    fileID = fopen('b7b_result_anwar.txt','w');
    fprintf(fileID, '%24.18e %24.18e %24.18e %24.18e %24.18e\n',Result3rdOct.');
    fclose(fileID);
    xlswrite('b7b_result_anwar.xls', Result3rdOct)
    
    fileID = fopen('b7b_result_latex.txt','w');
    fprintf(fileID, '\\begin{tabular}{|l|c|c|c|c|}\\hline\n');
    fprintf(fileID, 'Freq. [Hz] & Source 0 [Pa$^2$] & Source 1 [Pa$^2$] & Source 2 [Pa$^2$] & Source 3 [Pa$^2$] \\\\ \\hline\n');
    fprintf(fileID, '%.2f & %.6f & %.6f & %.6f & %.6f \\\\ \\hline\n',Result3rdOct.');
