						...
						int deStrategy,double diffScale,double crossoverProb)
{
	...
	switch (strategy)
	{
		...
		case stMyCase:
			calcTrialSolution = &DESolver::MyCase;
			break;
	}
...
bool DESolver::Solve(int maxGenerations, double *min,double *max)
{
	int generation;
	int candidate;
	bool bAtSolution;
	...
	for (generation=0;(generation < maxGenerations) && !bAtSolution;generation++)
		...
		{
			(this->*calcTrialSolution)(candidate, min, max);
...
void DESolver::Best1Exp(int candidate, double* min, double* max)
{
	int r1, r2;
	int n;
...
void DESolver::Rand1Exp(int candidate, double* min, double* max)
{
	int r1, r2, r3;
	int n;
...
void DESolver::RandToBest1Exp(int candidate, double* min, double* max)
{
	int r1, r2;
	int n;
...
void DESolver::Best2Exp(int candidate, double* min, double* max)
{
	int r1, r2, r3, r4;
	int n;
...
void DESolver::Rand2Exp(int candidate, double* min, double* max)
{
	int r1, r2, r3, r4, r5;
	int n;
...
void DESolver::Best1Bin(int candidate, double* min, double* max)
{
	int r1, r2;
	int n;
...
void DESolver::Rand1Bin(int candidate, double* min, double* max)
{
	int r1, r2, r3;
	int n;
...
void DESolver::RandToBest1Bin(int candidate, double* min, double* max)
{
	int r1, r2;
	int n;
...
void DESolver::Best2Bin(int candidate, double* min, double* max)
{
	int r1, r2, r3, r4;
	int n;
...
void DESolver::Rand2Bin(int candidate, double* min, double* max)
{
	int r1, r2, r3, r4, r5;
	int n;
...
void DESolver::MyCase(int candidate, double* min, double* max)
{
	int r1, r2, r3;
	int n;
	...
	for (int i=0; (RandomUniform(0.0,1.0) < probability) && (i < nDim); i++)
	{
		trialSolution[n] = Element(population,r1,n)
							+ scale * (Element(population,r2,n)
							- Element(population,r3,n));
        if ((trialSolution[n]>max[n])||(trialSolution[n]<min[n]))
            trialSolution[n] = Element(population,candidate,n);
		n = (n + 1) % nDim;
	}
